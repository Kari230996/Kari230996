# Hi, I'm Karina 👋  

Beginner Python Backend Developer with 9 months of experience

I develop backend applications and REST APIs using Python.  
I have hands-on experience with Django and FastAPI, databases, background tasks, and containerized environments.

---

### 🛠 Tech Stack
![Python](https://img.shields.io/badge/-Python-000?logo=python)
![Django](https://img.shields.io/badge/-Django-000?logo=django)
![Django REST Framework](https://img.shields.io/badge/-DRF-000?logo=django)
![FastAPI](https://img.shields.io/badge/-FastAPI-000?logo=fastapi)
![PostgreSQL](https://img.shields.io/badge/-PostgreSQL-000?logo=postgresql)
![SQLAlchemy](https://img.shields.io/badge/-SQLAlchemy-000?logo=sqlalchemy)
![Redis](https://img.shields.io/badge/-Redis-000?logo=redis)
![Celery](https://img.shields.io/badge/-Celery-000?logo=celery)
![Docker](https://img.shields.io/badge/-Docker-000?logo=docker)
![Git](https://img.shields.io/badge/-Git-000?logo=git)
![Postman](https://img.shields.io/badge/-Postman-000?logo=postman)
![Swagger](https://img.shields.io/badge/-Swagger-000?logo=swagger)
![Linux](https://img.shields.io/badge/-Ubuntu-000?logo=ubuntu)
![HTML](https://img.shields.io/badge/-HTML5-000?logo=html5)
![CSS](https://img.shields.io/badge/-CSS3-000?logo=css3)
![JavaScript](https://img.shields.io/badge/-JavaScript-000?logo=javascript)

---

### 🚀 What I Do
- Build REST APIs using **Django REST Framework** and **FastAPI**  
- Work with PostgreSQL via **Django ORM** and **SQLAlchemy**  
- Implement background tasks with **Celery + Redis**  
- Containerize applications using **Docker**  
- Document and test APIs with **Swagger** and **Postman**  
- Use **Git** for version control and **Ubuntu** as a development environment  

---

### 📌 Experience
- **Python Backend Developer** — 9 months of practical experience  
- Developed pet and test projects with real backend logic and APIs  

---

### 📫 Contact me
- Telegram: https://t.me/karina_api
- Email: karina.apaeva96@gmail.com  

---

